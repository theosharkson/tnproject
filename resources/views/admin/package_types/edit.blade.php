        ...
        <div class="col-md-5">
            ...
            <div class="panel panel-danger add-shadow" data-sortable-id="form-stuff-1">
                ...
                <div class="panel-body">
                    <form action="{{route('package-types.update',['packageType'=>$packageType->id])}}" class="form-horizontal" method="POST" enctype="multipart/form-data"
                    ...
                    {{ csrf_field() }} 
                        ...
                        @if($packageType->image)
                            <div class="form-group">
                                <img class="center-img"  src="{{route('package-type.images.thumb',['image'=>$packageType->image]) }}">
                            </div> 
                        @endif
                                ...
                                <select name="category" class="select2 form-control">
                                    <option value=""  selected>Choose</option>
                                      @foreach(getTypeCategories() as $key => $category)
                                          <option 
                                          @if(!empty(old('category')) and old('category') == $key)
                                              selected="" 
                                          @elseif($packageType->category == $key)
                                              selected="" 
                                          @endif
                        ...
                        <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-9">
                                ...
                                placeholder="" 
                                name="name" 
                                value="{{old('name')?old('name'):$packageType->name}}"
                                required="">
                        ...
                        <div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-9">
                                <textarea class="form-control" name="description" rows="5">{{old('description')?old('description'):$packageType->description}}</textarea>
