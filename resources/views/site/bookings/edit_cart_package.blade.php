...
<section class="page-title smaller-title bg-overlay-black-60 parallax"  style="background: url({{route('packages.images.raw',['image'=>$orderPackage->package->image]) }}) repeat;"
  data-jarallax='{"speed": 0.7}'>
  <div class="container">
    <div class="row"> 
     <div class="col-lg-12"> 
      <div class="page-title-name">
          <h1><i class="fa fa-shopping-cart icon"></i> Your Cart <i class="fa fa-angle-double-right icon"></i> {{$orderPackage->package->name}}</h1>
           ...
           <div class="col-lg-12 col-md-12">
             <div class="product-detail clearfix">
              <div class="product-detail-title mb-20 sm-mt-40 text-center">
                <h4 class="mb-10 ">{{$orderPackage->package->type->name}} <br> {{$orderPackage->package->name}}</h4>
                ...
                <div class="product-detail add-to-cart">
                 ...
                 <div class="row">
                   ...
                   <div class="col-sm-6">
                     <div class="product-detail-des mb-30">
                        <ul class="list list-unstyled list-arrow">
                          @php
                             $products = preg_split('/\n|\r\n?/', $orderPackage->package->description);
                             // dd($options);
                           @endphp
                   ...
                   <div class="col-sm-6">
                     ...
                     style="background-image: url({{asset('site-assets/images/pattern/bg-pattern-1.jpg')}});">
                        ...
                        <h3 class="pl-10"><mark>{{getCurrencySymbol()}} {{currency($orderPackage->package->$county_price)}}</mark></h3>
                        <input type="hidden" id="base_price" value="{{$orderPackage->package->$county_price}}">
                      </div>
                   </div>
                 ...
                 <div class="product-detail-meta">
                   <div class="row mb-20">
                       ...
                       <div class="sidebar-widgets-wrap">
                         <div class="sidebar-widget mb-40">
                           ...
                           @foreach($extras as $key => $extra)
                             ...
                             <div class="recent-item clearfix" id="extra_item_{{$extra->id}}" 
                              @if(in_array($extra->id, $orderPackage->orderPackageExtras()->pluck('extra_id')->toArray() ))
                                style="display: none;" 
                              @endif
                              >
                                <div class="row">
                                  <div class="col-sm-8 m-0">
                                    <div class="recent-image">
                                        <a href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-sm-{{$extra->id}}">
                                          <img class="img-responsive" src="{{route('extras.images.thumb',['image'=>$extra->image]) }}" alt="">
                                        </a>
                                    </div>
                                    <div class="recent-info">
                                        <div class="recent-title">
                                             <a href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-sm-{{$extra->id}}">{{$extra->name}}</a> 
                                        </div>
                                  ...
                                  <div class="col-sm-4">
                                    <a href="javascript:void(0)" style="font-size: 10px;" 
                                        ...
                                        onclick="$('#extra_item_{{$extra->id}}').hide('slow'); $('html, body').animate({scrollTop: $('#all_added_extras').offset().top-200}, 500);  $('#extras_content_{{$extra->id}} .cart-item').clone().prop('id', 'added_extra_{{$extra->id}}').appendTo($('#user_extras')); calculateTotal()">
                                         add extra 
                                       <i class="fa fa-long-arrow-right"></i> 
                                     </a>

                                     <div id="extras_content_{{$extra->id}}" style="display: none;">
                                       ...
                                       <div class="cart-item animated bounceInRight">
                                         ...
                                         <div class="cart-close">
                                             <p 
                                                ...
                                                onclick="removeElement('#added_extra_{{$extra->id}}'); showElement('#extra_item_{{$extra->id}}');">
                                  ...
                                  <div class="modal fade bs-example-modal-sm-{{$extra->id}}" tabindex="-1" role="dialog" >
                     ...
                     <form method="POST" id="order_form" action="{{ route('update-cart-item',['order_package'=>$orderPackage->id]) }}" >
                      @csrf

                      <input type="hidden" name="package_id" value="{{$orderPackage->package->id}}">
                       <div class="col-lg-6 col-md-6" id="all_added_extras">
                          ...
                          <div class="row" id="user_extras">
                            ...
                            @foreach($orderPackage->orderPackageExtras as $key => $orderExtra)
                                @php 
                                  $qty = $orderExtra->quantity?$orderExtra->quantity:1;
                                @endphp
                                <div class="cart-item animated bounceInRight" id="added_extra_{{$orderExtra->extra->id}}">
                                  <div class="cart-image">
                                    <img class="img-responsive" src="{{route('extras.images.thumb',['image'=>$orderExtra->extra->image]) }}" alt="">
                                  </div>
                                  <div class="cart-name clearfix">
                                    <strong style="padding-left:0px;">
                                    ...
                                    <div class="cart-price">
                                      <ins>{{getCurrencySymbol()}} {{currency($orderExtra->extra->$county_price)}}</ins>
                                   </div>
                                  </div>
                                  @if($orderExtra->extra->type == 'deliverable')
                                    <div class="cart-name clearfix">
                                      <input 
                                        style="width: 90px; position: absolute; right: 67px;" 
                                        type="number" 
                                        class="form-control extra_qty_{{$orderExtra->extra->id}}" 
                                        name="extras[{{$orderExtra->extra->id}}][quantity]"
                                        placeholder="Qty"
                                        value="{{$qty}}"
                                        onkeyup="calculateExtraRow('extra_base_price_{{$orderExtra->extra->id}}', 'extra_price_{{$orderExtra->extra->id}}', 'extra_qty_{{$orderExtra->extra->id}}');" 
                                        onchange="calculateExtraRow('extra_base_price_{{$orderExtra->extra->id}}', 'extra_price_{{$orderExtra->extra->id}}', 'extra_qty_{{$orderExtra->extra->id}}');" 
                                        onmouseup="calculateExtraRow('extra_base_price_{{$orderExtra->extra->id}}', 'extra_price_{{$orderExtra->extra->id}}', 'extra_qty_{{$orderExtra->extra->id}}');"
                                        min="1">

                                        <input 
                                        type="hidden"
                                        class="extra_base_price_{{$orderExtra->extra->id}}"
                                        value="{{$orderExtra->extra->$county_price}}">
                                    </div>
                                  @endif
                                  <input 
                                    type="hidden"
                                    value="{{$orderExtra->extra->id}}"
                                    name="extras[{{$orderExtra->extra->id}}][extra_id]">
                                    <input 
                                    type="hidden"
                                    class="extra_price extra_price_{{$orderExtra->extra->id}}"
                                    value="{{$orderExtra->extra->$county_price * $qty}}">
                                          
                                  <div class="cart-close">
                                      <p 
                                         style="font-size: 35px;"
                                         onclick="removeElement('#added_extra_{{$orderExtra->extra->id}}'); showElement('#extra_item_{{$orderExtra->extra->id}}');">
                                        <i class="fa fa-times-circle"></i> 
                                      </p>
                                   </div>
                                </div>
                   ...
                   <div class="row">
                     <div class="col-sm-12">
                       ...
                       >
                          ...
                          <h1 class="pl-10">{{getCurrencySymbol()}} <mark id="total_price">{{currency($orderPackage->package->$county_price)}}</mark></h1>
                        </div>
                     </div>
                     <div class="col-sm-12 text-center">
                      <button 
                        ...
                        <span> Update Cart <i style="font-size: 20px; line-height: 10px;" class="ti-shopping-cart"></i></span> 
                      </button>
                     </div>
                   </div>
